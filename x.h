...
struct Particle {
  ...
  short motherIndex;
  int pdgId;
  float vx; //x-component of vertex 
  float vy;
  float vz;
  float pt;
  float eta;
...
PNode*
setDaughters(short _index, std::vector<Particle>& _particles, float _minPt)
{
  Particle& gen(_particles[_index]);
  ...
  node->mass = gen.mass;;
  node->pt = gen.pt;
  node->eta = node->pt > 0. ? gen.eta : (gen.pz > 0. ? 10000. : -10000.);
  node->phi = gen.phi;
...
void
viewGenTreeRA3(vector<Particle>& _particles, float _minPt, bool _showMass = false)
{
   std::vector<PNode*> rootNodes;

   //std::vector<Particle>& particles(_event.genParticles);

   for(short iP(0); iP < short(_particles.size()); ++iP){
     Particle& gen(_particles[iP]);

     if(gen.motherIndex == -1){
       PNode* node(setDaughters(iP, _particles, _minPt));
