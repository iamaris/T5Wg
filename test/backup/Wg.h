...
#ifdef Wg_cxx
Wg::Wg(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("WgBNEW.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("WgBNEW.root");
      }
...
Int_t Wg::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Wg::LoadTree(Long64_t entry)
{
