...
#ifdef j2g_cxx
j2g::j2g(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("j2gNEW.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("j2gNEW.root");
      }
...
Int_t j2g::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t j2g::LoadTree(Long64_t entry)
{
