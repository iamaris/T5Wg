...
void skimmerJE(TString const& _input,TString const& _out="j2eNEW")
{
  ...
  while(allObjects.GetEntry(iEntry++) != 0){
    ...
    if(lE.size()!=0) {  
      for(std::set<unsigned>::iterator xx=lE.begin();xx!=lE.end();++xx)
         if(!CheckTightElectron(re,*xx))//Fail Tight (added!, was missed before)
         tE.insert(*xx); 
    }
